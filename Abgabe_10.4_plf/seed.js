...
async function main() {
  ...
  const ZooIds = (await prisma.Zoo.findMany({ select: { cuid: true } })).map(_ => _.cuid);
...
for (let zooId of ZooIds) {
  ...
  for (let i = 0; i < abteilungenCount; i++) {
    ...
    const AbteilungsIds = (await prisma.Abteilung.findMany({ select: { cuid: true } })).map(_ => _.cuid);
    ...
    const MitarbeiterIds = (await prisma.mitarbeiter.findMany( {select: {cuid: true}})).map(_ => _.cuid);
        
    for (let mitarbeiterId of MitarbeiterIds) {
    const abteilungenCount = faker.number.int({ min: 1, max: 4 });

        for (let i = 0; i < abteilungenCount; i++) {
            const abteilungId = AbteilungsIds[faker.number.int({ min: 0, max: AbteilungsIds.length - 1 })];

            await prisma.mitarbeiter.update({
            where: { cuid: mitarbeiterId },
            data: {
                Abteilungen: {
                connect: { cuid: abteilungId },
                },
            },
            });
        }
